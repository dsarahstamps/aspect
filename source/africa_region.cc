...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    private:
      /**
       * Pointer to an object that describes the geometry.
       */
      Step53::AfricaGeometry   manifold;
      ...
      static void clear_manifold_ids (Triangulation<dim> &triangulation)
	{
      ...
      static void write_mesh (Triangulation<dim> &triangulation)
        {
        static int i=0;
        std::string s = "grid-" + Utilities::int_to_string(i) + ".vtu";
        std::ofstream o (s.c_str());
        GridOut().write_vtu (triangulation, o);
        }
    };
    ...
    Africa<3>::create_coarse_mesh(parallel::distributed::Triangulation<3> &coarse_grid) const
    {
      ...
      GridTools::transform (std_cxx11::bind(&Step53::AfricaGeometry::push_forward,
	...
	for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
      ...
      coarse_grid.signals.post_refinement.connect (std_cxx11::bind (&clear_manifold_ids,
								    std_cxx11::ref(coarse_grid)));   
      coarse_grid.signals.post_refinement.connect (std_cxx11::bind (&write_mesh,
                                                                    std_cxx11::ref(coarse_grid)));
    }
