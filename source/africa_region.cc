...
namespace aspect
{
  namespace InitialConditions
  {
    ...
    template <int dim>
        ...
        if (input1.is_open())
          {
            ...
            // Start loop with int count to calculate delta below
            int count = 0;
            while (true)
              {
      ...
      //End of class that sets initial temperature condition

    /**
     * A class that defines crustal region.
     *
     * @ingroup InitialConditionsModels
     */
    template <int dim>
      class CrustalRegion : public Interface<dim>
      {
      public:
    	double delta;
    	int crust_flag;
    ...
    template <int dim>
      void
      CrustalRegion<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Initial conditions");
          {
            prm.enter_subsection("Crustal region");
              {
                prm.declare_entry("Isotherm filename",
                                  "thickness.txt",
                                  Patterns::FileName(),
    ...
    template <int dim>
      void
      CrustalRegion<dim>::parse_parameters(ParameterHandler &prm)
    ...
    /** End of class that defines crustal region.
     *
     */
  }
}
...
namespace aspect
{
  namespace InitialConditions
  {
    ASPECT_REGISTER_INITIAL_CONDITIONS(CrustalRegion,
                                       "crustal region",
                                       "The extent of the crustal region. "
                                       "If region is crustal assign Coulomb friction law. ")
  }
}
...
namespace aspect
{
  namespace MaterialModel
  {
               ...
               const Point<dim> &position) const
    {
        ...
        const double B_diff = 810000000000.0; // see p.248-240 Shubert et al., 2004; grain size = 3 mm
        const double R = 8.3144;                        // gas constant J/K.mol
        const double V_diff = 0.000006;         // Activation volume m^3/mol
        const double E_diff = 300000;           // J/mol
